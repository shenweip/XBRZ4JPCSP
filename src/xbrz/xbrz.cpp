...
void xbrz::scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
	...
	{
		case 2:
			return unnamed::scaleImage<unnamed::Scaler2x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
		case 3:
			return unnamed::scaleImage<unnamed::Scaler3x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
		case 4:
			return unnamed::scaleImage<unnamed::Scaler4x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
		case 5:
			return unnamed::scaleImage<unnamed::Scaler5x>(src, trg, srcWidth, srcHeight, cfg, yFirst, yLast);
	}
	assert(false);
}
...
{
	return unnamed::colorDist(col1, col2, luminanceWeight) < equalColorTolerance;
}
	...
	switch (st)
	{
		case NN_SCALE_SLICE_SOURCE:
			...
			for (int y = yFirst; y < yLast; ++y)
			{
				...
				if (blockHeight > 0)
				{
					const uint32_t* srcLine = unnamed::byteAdvance(src, y * srcPitch);
					uint32_t* trgLine  = unnamed::byteAdvance(trg, yTrg_first * trgPitch);
					int xTrg_first = 0;
					...
					for (int x = 0; x < srcWidth; ++x)
					{
						...
						if (blockWidth > 0)
						{
							...
							unnamed::fillBlock(trgLine, trgPitch, srcLine[x], blockWidth, blockHeight);
							trgLine += blockWidth;
						}
					}
				}
		...
		case NN_SCALE_SLICE_TARGET:
			...
			for (int y = yFirst; y < yLast; ++y)
			{
				uint32_t* trgLine = unnamed::byteAdvance(trg, y * trgPitch);
				const int ySrc = srcHeight * y / trgHeight;
				const uint32_t* srcLine = unnamed::byteAdvance(src, ySrc * srcPitch);
